...
void AD_Init(void){
	...
	DMA_InitStruct.DMA_BufferSize=1;//设置传输计数器（缓存器）
	...
	GPIO_Init(GPIOA,&GPIO_InitStruct);//配置PA引脚为模拟输入引脚
	
	ADC_RegularChannelConfig(ADC1,ADC_Channel_1,1,ADC_SampleTime_1Cycles5);//配置规则组菜单，ADC1通道0，序列1，采样时间为55.5个ADCCLK周期
	...
	ADC_InitStruct.ADC_NbrOfChannel=1;//扫描序列数为1，多次扫描仅在扫描模式有用
	ADC_InitStruct.ADC_ScanConvMode=DISABLE;//确定非扫描模式
